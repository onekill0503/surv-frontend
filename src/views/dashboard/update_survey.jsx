...
const UpdateSurvey = (props) => {
  ...
  useEffect(() => {
    if(user_id){
      axios.get(`${config.apiUrl}/form/${slug}` , {headers:{
        'Authorization': `Bearer ${token}`
      }}).then(res => {
        if(res.status === 200){
          if(user_id === res.data.data.formData.user_id){
            setSurveyName(res.data.data.formData.title)
            setSurveyDescription(res.data.data.formData.description)
          }else{
  ...
  const saveUpdate = () => {
    axios.patch(`${config.apiUrl}/form` , {slug:slug , title: surveyName , description: surveyDescription} , {headers:{
      'Authorization': `Bearer ${token}`
    }}).then(res => {
      if(res.status === 200){
  ...
  return (
    <DashboardLayout>
      <div className="dashboard-title">Update Survey</div>
      ...
      <button className="mt-20 px-5 py-3 bg-violet-500 text-white rounded" onClick={e => saveUpdate()}>Save Update</button>
    </DashboardLayout>
  );
};
...
export default connect(stateToProps)(UpdateSurvey);
